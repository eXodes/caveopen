 ...
 * V40, V41, V42, V75, V78
 */
import { readFileSync, writeFileSync, existsSync, mkdirSync, copyFileSync, readdirSync, realpathSync } from "node:fs"
...
/**
 * Format a symbol with optional ANSI color. Exported for testing. V75
...
/**
 * Color a label token for output. V75, V78
 * green: added | registered | configured  yellow: updated
 */
export function colorLabel(label: string, tty: boolean): string {
  if (!tty) return label
  if (["added", "registered", "configured"].includes(label)) return `\x1b[32m${label}\x1b[0m`
  if (label === "updated") return `\x1b[33m${label}\x1b[0m`
  return label
}

/** Blue text when TTY; plain otherwise. V86 */
export function blue(s: string, tty: boolean): string {
  return tty ? `\x1b[94m${s}\x1b[0m` : s
}

const printOk   = (msg: string) => console.log(`${fmtSymbol("ok",   isTTY)} ${msg}`)
const printWarn = (msg: string) => process.stderr.write(`${fmtSymbol("warn", isTTY)} ${msg}\n`)
const printFail = (msg: string) => { process.stderr.write(`${fmtSymbol("fail", isTTY)} ${msg}\n`); process.exit(1) }

// ── JSONC strip (V21, V22, V23) ───────────────────────────────────────────
/** String-aware JSONC comment strip. Preserves // and /* inside quoted strings. V22 */
...
/** Strip comments + trailing commas, then JSON.parse. V21–V23 */
 ...
 * Char-scan to find array bounds, splice new JSON fragment. V72
 */
export function splicePluginArray(raw: string, newPlugins: unknown[]): string {
  ...
  const arrEnd = i
  return raw.slice(0, arrStart) + JSON.stringify(newPlugins) + raw.slice(arrEnd + 1)
}

/**
 * Surgically inject `"cavemem": entry` into the `"mcp"` object in raw JSONC.
 * Char-scan to mcp object bounds; insert before closing }. V73
 */
export function spliceMcpCavemem(raw: string, entry: unknown): string {
  ...
  // V74: empty or trailing-comma → no separator; else prepend ", "
  const insertion = inner.trim() === "" || inner.trimEnd().endsWith(",") ? entryStr : `, ${entryStr}`
  return raw.slice(0, objEnd) + insertion + raw.slice(objEnd)
}

// ── Entry point guard — run only when executed directly (V40, V42) ────────
function runCLI(): void {
  // ── Subcommand gate (V40, V42) ──────────────────────────────────────────
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i]!
    if (a === "--modes" && args[i + 1]) { modes = args[++i]!; continue }
    if (a === "--project") { target = "project"; continue }
    if (a === "--global") { target = "global"; continue }
    if (a === "--dry-run") { dryRun = true; continue }
    printFail(`Unknown arg: ${a}`)
  }

  const scope = target                                  // "global" | "project" — V86
  const g = (s: string) => blue(s, isTTY)

  // ── Paths ──────────────────────────────────────────────────────────────
  const opencodeDir = target === "global"
  ...
  // Track pre-existing caveopen entry for ADDED|MODIFIED state (V78)
  const hadCaveopen = (config.plugin as unknown[]).some(
    e => e === "caveopen" || (Array.isArray(e) && e[0] === "caveopen")
  )

  // V40: npm-form — "caveopen" or ["caveopen", {"modes":"..."}]; idempotent
  ...
  // V73: inject mcp.cavemem when cavemem mode included
  const mcpEntry = { type: "local", command: ["npx", "cavemem", "mcp"] }
  const includeCavemem = !modes || modes.split(",").map(m => m.trim()).includes("cavemem")
  ...
  // V72: surgical splice — preserve JSONC comments; ⊥ JSON.stringify(config) roundtrip
  ...
  // ── Write config + per-mutation output (V75, V78, V86) ──────────────────
  try {
    if (!dryRun) writeFileSync(jsonPath, outputStr)
    const pluginLabel = hadCaveopen ? "updated" : "registered"
    // V87 config format: {colorAction}  {blue(type)} {name} → {blue(scope+":config")} type (plain)
    printOk(`${colorLabel(pluginLabel, isTTY)}  ${g("plugin")} caveopen → ${g(scope + ":config")} plugin`)
    if (needsMcp) {
      printOk(`${colorLabel("configured", isTTY)}  ${g("mcp")} cavemem → ${g(scope + ":config")} mcp`)
    }
  } catch (e) {
  ...
  const activeMods = new Set<string>(
    modes ? modes.split(",").map(m => m.trim()).filter(Boolean) : ["caveman", "cavekit", "cavemem"]
  )
  const hasCaveman = activeMods.has("caveman")
  const hasCavekit = activeMods.has("cavekit")

  // ── Per-file copy helpers (V78) ─────────────────────────────────────────

  /**
   * Copy src → dest with state detection and V86 per-file output.
   * label: added (dest ⊥ exist) | updated (dest existed). ∀ case → copyFileSync.
   * V86 asset format: {colorAction}  {blue(type)} {name} → {blue(scope:type)} {name}
   */
  function copyFileWithState(srcPath: string, destPath: string, type: string, name: string): void {
    const label: "added" | "updated" = existsSync(destPath) ? "updated" : "added"
    if (!dryRun) {
      mkdirSync(dirname(destPath), { recursive: true })
      copyFileSync(srcPath, destPath)
    }
    printOk(`${colorLabel(label, isTTY)}  ${g(type)} ${name} → ${g(`${scope}:${type}`)} ${name}`)
  }

  /** Recursively copy srcDir → destDir, emitting per-file V86 output. type+name constant across subtree. */
  function walkCopy(srcDir: string, destDir: string, type: string, name: string): void {
    ...
    for (const e of readdirSync(srcDir, { withFileTypes: true })) {
      if (e.isDirectory()) {
        ...
        try {
          copyFileWithState(join(srcDir, e.name), join(destDir, e.name), type, name)
        } catch (err) {
          printWarn(`copy ${e.name}: ${err instanceof Error ? err.message : String(err)}`)
        }
      }
    }
  }

  // ── Copy assets (V41, V75, V76, V78: module-scoped, per-file output) ────
  if (!dryRun) {
  ...
  // Skills: caveman→ caveman*/cavecrew/ dirs; cavekit→ ck-*/ dirs (V76)
  try {
  ...
  // Commands: caveman→ caveman*.md; cavekit→ ck:*.md (V76)
  try {
    ...
    for (const e of readdirSync(src, { withFileTypes: true })) {
      ...
      if (!include) continue
      try {
        copyFileWithState(join(src, n), join(commandsDir, n), "commands", "/" + n.replace(/\.md$/, ""))
      } catch (err) {
        printWarn(`copy ${n}: ${err instanceof Error ? err.message : String(err)}`)
      }
    }
  } catch (e) {
  ...
  // FORMAT.md: cavekit only (V76, V25)
  if (hasCavekit) {
    try {
      copyFileWithState(join(assetsDir, "FORMAT.md"), join(opencodeDir, "plugins", "caveopen", "FORMAT.md"), "plugins", "caveopen/FORMAT.md")
    } catch (e) {
      printWarn(`copy FORMAT.md: ${e instanceof Error ? e.message : String(e)}`)
    }
  }

  // README.md: always (V89 — not module-gated; always useful)
  try {
    copyFileWithState(join(assetsDir, "README.md"), join(opencodeDir, "plugins", "caveopen", "README.md"), "plugins", "caveopen/README.md")
  } catch (e) {
    printWarn(`copy README.md: ${e instanceof Error ? e.message : String(e)}`)
  }

  // Agents: caveman→ cavecrew-*.agent.md (V76)
  if (hasCaveman) {
    try {
      ...
      for (const e of readdirSync(src, { withFileTypes: true })) {
        ...
        try {
          copyFileWithState(join(src, e.name), join(agentsDir, e.name), "agents", e.name.replace(/\.agent\.md$/, ""))
        } catch (err) {
          printWarn(`copy ${e.name}: ${err instanceof Error ? err.message : String(err)}`)
        }
      }
...
try { __realArgv1 = realpathSync(process.argv[1]) } catch { /* ENOENT — keep original */ }
