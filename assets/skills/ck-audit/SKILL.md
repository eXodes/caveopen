...
Read `SPEC.md §V`. Note today's date. Note §B entries (bug backlog — these mark invariants with precedent).
...
- One suggested next step: `spec amend §V` (to prune DEAD) or `bug:` referencing NEEDS TEST entries
