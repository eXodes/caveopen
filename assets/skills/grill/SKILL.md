...
Climb in order. Each rung: use `question` tool with **one** question + recommended answer as the first option.
...
`question` tool options: recommended answer first, alternatives after, "other" always available.

> Q: auth — session cookie or JWT?
> (a) JWT — stateless, fits your horizontal scaling §C (rec)
> (b) session cookie
> (c) other
